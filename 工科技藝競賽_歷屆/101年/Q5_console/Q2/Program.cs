...
class Program
{
    static void Main(string[] args)
    {
        ...
        while (bfs.Count > 0)
        {
            ...
            // Check goal
            if (endstate.Contains(now.State))
            {
                now.Answer = now.State;
                result = now;
                break;    // <-- must break the BFS!
            }

            // Expand
            foreach (var nxt in move(now))
            {
                if (!visited.Contains(nxt.State))
                {
        ...
        Console.WriteLine($"You Final Answer : {result.State}");
